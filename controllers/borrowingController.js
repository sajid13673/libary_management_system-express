...
const createBorrowing = async (req, res) => {
  try {
    ...
    const hasActiveBorrowings = book.borrowings.some(
    ...
    if (hasActiveBorrowings) {
      res
        ...
        .json({
          status: false,
          message: "Book already has an active borrowing",
        });
      return;
    }
    ...
    res
      .status(201)
      .json({ status: false, message: "Borrowing created successfully" });
  } catch (err) {
    res.status(500).json({ status: false, message: err.message });
  }
};
...
const deleteBorrowing = async (req, res) => {
  try {
    ...
    if (borrowing.fine && !borrowing.fine.isPaid) {
      res
        .status(400)
        .json({ status: false, message: "This borrowing has unpaid fine" });
      return;
    }
    await borrowing.destroy();
    res
      .status(200)
      .json({ status: true, message: "Borrowing deleted successfully" });
  } catch (err) {
    res.status(500).json({ status: false, message: err.message });
  }
};
...
const updateBorrowing = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Borrowing updated successfully" });
  } catch (err) {
    res.status(500).json({ status: false, message: err.message });
  }
};
